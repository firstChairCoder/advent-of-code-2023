...
const data = fs.readFileSync(
  path.resolve(__dirname, "../inputs/day23.txt"),
  "utf-8"
);
...
const getGraph = () => {
  const addConnectNode = (curr) => {
    // try to locate existing one
    let newJuncId = nodes.findIndex(
      (n) => n.p[0] == curr.p[0] && n.p[1] == curr.p[1]
    );

    if (newJuncId == curr.lastJuncId) return newJuncId;

    if (newJuncId == -1)
      newJuncId = nodes.push({ p: curr.p.slice(), connections: [] }) - 1;

    // we need to connect cur.lastJuncId and newJuncId
    if (
      nodes[curr.lastJuncId].connections.findIndex(
        (conn) => conn.id == newJuncId
      ) == -1
    )
      nodes[curr.lastJuncId].connections.push({
        id: newJuncId,
        distance: curr.steps - curr.stepsToLastJunc,
      });

    if (
      nodes[newJuncId].connections.findIndex(
        (conn) => conn.id == curr.lastJuncId
      ) == -1
    )
  ...
  let stack = [
      { p: startPos.slice(), steps: 0, lastJuncId: 0, stepsToLastJunc: 0 },
    ],
...
//PART2: convert map to graph and dfs again
const part2 = () => {
  let nodes = getGraph();
...
//PART1: brute force dfs with remembering seen fields on path, sped up by traversing the narrow paths quickly
const part1 = () => {
  const getMoves = (cur) => {
    let moves = [],
